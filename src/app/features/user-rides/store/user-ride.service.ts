...
export class UserRideService {
  ...
  async getCurrentRide(): Promise<void> {
    try {
      const currentRide = await this.http.get<Ride>(API_RESSOURCE_URI.CURRENT_USER_RIDE);
      this.userRideStore.update({currentRide});
    } catch (e) {
      this.snackBar.warnning('Erreur lors de la récupération du trajet en cours ' + e.error.error);
    }
  }

  async startRide(bike: Bike, startStation: Station): Promise<void> {
    try {
      const result = await this.http.post<Ride>(API_RESSOURCE_URI.USER_START_RIDE, {
        bike,
        startStation
      });
      ...
      this.snackBar.warnning('Erreur lors de la création du trajet en cours ' + e.error.error);
    }
  }

  async endRide(ride: Ride, endStation: Station, length: number): Promise<void> {
    try {
      ...
      this.snackBar.warnning('Erreur lors de la cloture du trajet en cours ' + e.error?.error);
    }
  }
